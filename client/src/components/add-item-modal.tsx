...
export function AddItemModal({ open, onClose, onSuccess, filterOptions, editingItem, initialValues }: AddItemModalProps) {
  ...
  const createItemMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      let errorMessage = `Impossible de ${editingItem ? 'modifier' : 'ajouter'} l'article.`;
      
      // Handle specific error types
      if (error.message?.includes('duplicate key')) {
        if (error.message.includes('code_barre')) {
          errorMessage = 'Ce code-barre existe déjà. Veuillez utiliser un code-barre unique.';
        } else if (error.message.includes('num_inventaire')) {
          errorMessage = 'Ce numéro d\'inventaire existe déjà. Un nouveau numéro sera généré automatiquement.';
        }
  ...
  const generateInventoryNumber = async () => {
    try {
      const response = await fetch("/api/generate-inventory-number");
      if (!response.ok) throw new Error("Failed to generate inventory number");
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto bg-[#ffffff]" aria-describedby="add-item-description">
        <DialogHeader>
          ...
          <p id="add-item-description" className="text-sm text-muted-foreground">
            {editingItem ? "Modifiez les informations de l'article existant." : "Remplissez les informations pour ajouter un nouvel article à l'inventaire."}
          </p>
        </DialogHeader>
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={async () => {
                          ...
                          field.onChange(inventoryNumber);
                        }}
                      >
                        Générer
                      </Button>
