...
export class DatabaseStorage implements IStorage {
  ...
  async createInventoryItem(item: InsertInventoryItem): Promise<InventoryItem> {
    // If no inventory number provided, generate one
    let inventoryNumber = item.num_inventaire;
    if (!inventoryNumber) {
      inventoryNumber = await this.generateUniqueInventoryNumber();
    }
    ...
    const [createdItem] = await db
      ...
      .values({
        ...item,
        num_inventaire: inventoryNumber,
  ...
  async generateUniqueInventoryNumber(): Promise<string> {
    const year = new Date().getFullYear();
    
    try {
      // Get the highest existing sequential number for the current year
      const result = await db
        .select({ 
          max_number: sql<number>`MAX(CAST(SUBSTRING(num_inventaire FROM 'INV-${year}-(\\d+)$') AS INTEGER))` 
        })
        .from(inventoryItems)
        .where(sql`num_inventaire ~ '^INV-${year}-\\d+$'`);
      
      let nextNumber = 1366; // Start from 1366 since we have 1365 items
      if (result[0]?.max_number && result[0].max_number >= nextNumber) {
        nextNumber = result[0].max_number + 1;
      }
      
      // Generate sequential number with safety check
      let attempts = 0;
      const maxAttempts = 100;
      
      while (attempts < maxAttempts) {
        const inventoryNumber = `INV-${year}-${nextNumber}`;
        
        // Double-check uniqueness
        const existing = await db
          .select()
          .from(inventoryItems)
          .where(eq(inventoryItems.num_inventaire, inventoryNumber))
          .limit(1);
