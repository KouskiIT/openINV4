...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/generate-inventory-number", apiRateLimit, async (req, res) => {
    try {
      const inventoryNumber = await storage.generateUniqueInventoryNumber();
      res.json({ inventoryNumber });
    } catch (error) {
      console.error("Error generating inventory number:", error);
      res.status(500).json({ message: "Failed to generate inventory number" });
    }
  });
  ...
  // Generate unique inventory number
  app.get("/api/generate-inventory-number", async (req, res) => {
    try {
      const inventoryNumber = await storage.generateUniqueInventoryNumber();
      res.json({ inventoryNumber });
    } catch (error) {
